import {getAdmin, verify, json, isAdmin, booking, audit, cleanText} from './_firebase-admin.mjs';
const transitions = {
...
export async function handler(event) {
  try {
    ...
    if (body.action === 'status') {
      ...
      if (!admin) {
        if (['approved', 'rejected', 'done'].includes(next) && !owner) return json(403, {error: 'פעולה לבעל הרכב בלבד'});
      ...
      if (next === 'done' && !admin) {
        if (!current.handover?.pickup || !current.handover?.return) return json(409, {error: 'יש להשלים תיעוד לפני נסיעה ותיעוד החזרה'});
      }
      await ref.update({status: next, done: next === 'done', updatedAt: Date.now()});
    ...
    if (body.action === 'handover') {
      ...
      if (body.stage === 'pickup' && current.status === 'approved') await ref.update({status: 'active', updatedAt: Date.now()});
