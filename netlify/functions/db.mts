...
async function getRecord(collection: string, rid: string) {
  const { rows } = await pool.query(`SELECT id, data FROM crown_records WHERE collection=$1 AND id=$2`, [collection, rid]);
  return rows[0] || null;
}
...
async function sendEmailMessage(to: string | string[], subject: string, html: string, text?: string) {
  ...
  if (!host || !user || !pass) {
    await pool.query(`INSERT INTO crown_records(collection,id,data) VALUES('email_queue',$1,$2::jsonb)`, [id("em"), JSON.stringify({ to: recipients, subject, html, text, status: "missing_smtp_env", createdAt: Date.now() })]);
...
export const handler = async (event: any) => {
  ...
  try {
    if (action === "list") {
      ...
      if (body.where && body.where.field) {
        ...
        if (op === "array-contains") {
          const q = await pool.query(`SELECT id,data FROM crown_records WHERE collection=$1 AND (data -> $2) @> $3::jsonb ORDER BY updated_at DESC`, [collection, field, JSON.stringify([value])]);
          rows = q.rows;
        } else {
          const q = await pool.query(`SELECT id,data FROM crown_records WHERE collection=$1 AND data ->> $2 = $3 ORDER BY updated_at DESC`, [collection, field, String(value)]);
          rows = q.rows;
        }
      } else {
    ...
    if (action === "add") {
      ...
      await pool.query(`INSERT INTO crown_records(collection,id,data) VALUES($1,$2,$3::jsonb)`, [collection, newId, JSON.stringify(data)]);
    ...
    if (action === "set" || action === "update") {
      ...
      if (action === "set" && !body.merge) {
        await pool.query(`INSERT INTO crown_records(collection,id,data) VALUES($1,$2,$3::jsonb) ON CONFLICT(collection,id) DO UPDATE SET data=EXCLUDED.data, updated_at=NOW()`, [collection, rid, JSON.stringify(data)]);
      } else {
        await pool.query(`INSERT INTO crown_records(collection,id,data) VALUES($1,$2,$3::jsonb) ON CONFLICT(collection,id) DO UPDATE SET data=crown_records.data || EXCLUDED.data, updated_at=NOW()`, [collection, rid, JSON.stringify(data)]);
      }
      return json({ ok: true });
    }
    ...
    if (action === "login") {
      ...
      if (!u.pass_hash || u.pass) await pool.query(`UPDATE crown_auth SET pass=NULL, pass_hash=$1, updated_at=NOW() WHERE uid=$2`, [hashPassword(pass), u.uid]);
      const token = await createSession(u.uid);
      return json({ user: { uid: u.uid, email: u.email }, token });
    }

    if (action === "logout") {
      if (body.token) await pool.query(`DELETE FROM crown_sessions WHERE token_hash=$1`, [sha(String(body.token))]);
      return json({ ok: true });
    }
    ...
    if (action === "resetPassword") {
      const email = String(body.email || "").toLowerCase().trim();
      if (email) await pool.query(`INSERT INTO crown_records(collection,id,data) VALUES('password_resets',$1,$2::jsonb)`, [id("pw"), JSON.stringify({ email, createdAt: Date.now(), status: "requested" })]);
