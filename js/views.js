...
export const ICON = {
  ...
  doc: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 3v5h5"/><path d="M19 8v11a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7z"/><path d="M9 13h6M9 17h4"/></svg>',
...
function carCard(car, manage = false, period = null, index = 99) {
  ...
  return `<article class="card car car-clean${notFit ? ' car-nofit' : ''}" data-car-open="${esc(car.id)}"${cardRole} aria-label="${esc(`${car.make || ''} ${car.model || ''} — פתיחת פרטים`)}"><div class="car-photo"><img ${carImgAttrs(carImage(car))} alt="${esc(`${car.make || ''} ${car.model || ''}`)}" loading="${eager ? 'eager' : 'lazy'}"${eager ? ' fetchpriority="high"' : ''} decoding="async" data-car-image><div class="car-badges">${availPill(car.status)}${newBadge(car)}</div>${car.videoUrl ? '<span class="has-video">▶ וידאו</span>' : ''}<button type="button" class="fav-btn ${favList().has(car.id) ? 'on' : ''}" data-fav="${esc(car.id)}" aria-pressed="${favList().has(car.id)}" aria-label="שמירה במועדפים">${HEART_ICON}</button></div><div class="car-body"><div class="car-title-row"><h3>${esc(car.make || '')} ${esc(car.model || '')}</h3>${rating ? `<span class="car-rate">★ <b>${rating.toFixed(1)}</b>${reviewCount ? ` <small>(${reviewCount})</small>` : ''}</span>` : ''}</div><!-- Seat count is the one decision-critical fact the ad was missing: a 5-seater and an 8-seater are
     different products, and "מיניוואן" only implies it. Ordered so that when the line wraps on a narrow
     card, fuel — the least load-bearing item — is what drops to the second line. -->
...
<div class="car-specs"><span>${esc(car.year || '—')}</span><span>${esc(type)}</span>${car.seats ? `<span>${esc(car.seats)} מקומות</span>` : ''}${car.fuel ? `<span>${esc(car.fuel)}</span>` : ''}</div>${rented && carFreesAt(car.id) ? `<div class="frees-note">🕐 מתפנה בערך: ${fmtDate(carFreesAt(car.id))}</div>` : ''}${periodHtml}<div class="car-foot"><div class="price-stack"><div class="price${onRequest ? ' price-ask' : ''}">${priceMain}</div>${priceAlt ? `<small class="price-alt">${priceAlt}</small>` : ''}</div><span class="car-go">${car.status === 'available' ? 'פרטים והזמנה' : car.status === 'rented' ? 'שריון מראש' : 'צפייה'} ←</span></div>${manageRow}</div></article>`;
}
...
export function bindCarButtons() {
  ...
  // Delete lives here, next to the other card actions, because the card is built here. The identical
  // binding inside adminDashboard() only ever covered the admin table — an owner had no way at all to
  // remove a car they had published, even though car-action has always permitted it (it 403s only when
  // the caller is neither the owner nor an admin).
  app().querySelectorAll('[data-car-delete]').forEach(button => button.onclick = async () => {
    const id = button.dataset.carDelete;
...
export function openCar(id) {
  ...
  // Reviews fold into the same quiet <details> row as the spec sheet and the availability calendar.
  // They are reference material: the average is what drives the decision, and that already sits in the
  // key-specs block next to the price. Keeping the full list expanded pushed the modal past 1900px and
  // made every block below the form read at the same visual weight.
  const reviewsHtml = `${reviews.length ? `<details class="reviews-more"><summary><span>${heCountF(reviews.length, 'ביקורת', 'ביקורות')}</span></summary><div class="reviews">${reviews.slice(0, 8).map(r => `<div class="review"><div class="review-head"><span class="review-stars">${stars(r.score)}</span><small>${fmtDate(r.createdAt)}</small></div>${r.review ? `<p>${esc(r.review)}</p>` : ''}</div>`).join('')}</div></details>` : ''}${rateNote}`;
...
// Detach the open chat thread's message listener when the router leaves #chats. Safe before the
// personal-area module has loaded (there is nothing open in that case) and safe to call twice.
export function teardownChat() { try { __appMod?.teardownChatThread?.(); } catch {} }
