...
r = await call(fn['profile-save'], 'r1', {action: 'push-register', token: 'fake-fcm-token-abcdefghijklmnop'});
check('רישום אסימון התראות', S(r) === 200 && Object.values(get('users/r1/pushTokens') || {}).some(t => t?.token === 'fake-fcm-token-abcdefghijklmnop'));
r = await call(fn['profile-save'], 'r1', {action: 'push-register', token: 'x'});
check('אסימון קצר מדי נדחה (400)', S(r) === 400);
...
const bId = B(r).id;
check('ההזמנה שומרת שם השוכר לבעל הרכב (פופ-אפ בקשה)', get(`bookings/${bId}/renterName`) === 'יוסי כהן');
r = await call(fn['booking-create'], 'r1', {carId, startAt: start, endAt: end, termsAccepted: true, requestId: 'test-booking-1'});
...
// Message deletion: only the sender (or an admin) may delete a message.
const delId = B(r).id;
r = await call(fn['message-send'], 'r1', {bookingId: bId, deleteId: delId});
check('אי אפשר למחוק הודעה של מישהו אחר (403)', S(r) === 403 && get(`messages/${bId}/${delId}`) !== undefined);
r = await call(fn['message-send'], 'o1', {bookingId: bId, deleteId: delId});
check('מוחקים הודעה משלכם', S(r) === 200 && get(`messages/${bId}/${delId}`) === undefined);
r = await call(fn['message-send'], 'r1', {bookingId: bId, text: 'הודעת שוכר למחיקה'});
const rDelId = B(r).id;
r = await call(fn['message-send'], 'a1', {bookingId: bId, deleteId: rDelId});
check('מנהל מוחק כל הודעה', S(r) === 200 && get(`messages/${bId}/${rDelId}`) === undefined);
